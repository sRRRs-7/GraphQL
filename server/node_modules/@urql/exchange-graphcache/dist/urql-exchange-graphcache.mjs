...
function getNode(e, t, r) {
  var i;
  var n = !L && "read" === D && S && w.commutativeKeys.has(S);
  for (var a = 0, o = w.optimisticOrder.length; a < o; a++) {
    var s = w.optimisticOrder[a];
    var u = e.optimistic[s];
    n = n && s !== S;
    if (u && (!n || !w.commutativeKeys.has(s)) && (!L || "write" === D || w.commutativeKeys.has(s)) && void 0 !== (i = u.get(t)) && r in i) {
      return i[r];
    }
  }
...
Store.prototype.keyOfEntity = function keyOfEntity(e) {
  if (x.current && e === x.current.parent) {
    return x.current.parentKey;
  }
        ...
        forward: function(e) {
          return f((function(e) {
            if ("mutation" === e.operation.kind && isOfflineError(e.error) && function isOptimisticMutation(e, t) {
              ...
              h.push(O.get(e.operation.context._instance) || e.operation);
              updateMetadata();
              return !1;
            }
            if ("mutation" === e.operation.kind && !e.error) {
              O.delete(e.operation.context._instance);
            }
            return !0;
          }))(s(e));
        }
